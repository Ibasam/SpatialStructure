...
## SCENARIOS
EXPE<-c(10311111, 30311111, 50311111, 70311111, #0-30% dispersal - No diversity - simple configuration
...
for (iEXPE in 1:length(EXPE)) {
  load(paste0("results/PHENOGENOTYPE/PHENOGENOTYPE",EXPE[iEXPE],"_50.RData"))
...
Mig_scn <- list()
for (iEXPE in 1:length(EXPE)) {
  load(paste0("results/DEMOGRAPHY/DEMOGRAPHY",EXPE[iEXPE],"_50.RData"))
...
cv_40<-array(,dim=c(npop,length(EXPE)))
cv_50<-array(,dim=c(npop,length(EXPE)))
cv_30<-array(,dim=c(npop,length(EXPE)))
pente<-array(,dim=c((nInit+nYears-1),npop,length(EXPE)))
...
for (pop in 1:npop) { 
  ...
  for (scn in 1:length(EXPE)) {
    ...
    cv_init[pop,scn]<-mean(abs(cv_gene)[1:5], na.rm=T)
    cv_50[pop,scn]<-mean(abs(cv_gene)[(nInit+nYears-5):(nInit+nYears)], na.rm=T)
    cv_40[pop,scn]<-mean(abs(cv_gene)[(nInit+nYears-15):(nInit+nYears-10)], na.rm=T)
    cv_30[pop,scn]<-mean(abs(cv_gene)[(nInit+nYears-25):(nInit+nYears-20)], na.rm=T)
    
    pente[,pop,scn]<-abs(abs(mean_gene)[2:50]-abs(mean_gene)[1:49])
...
# gap <- c(-.15,0,.15)
# for(y in c(30,40,50)) {
#   ltm_nodiv <- (eval(parse(text=paste0("ltm_","",y)))[,scn_nodiv])-(ltm_init[,scn_nodiv[1]]) #no dispersal
#   ltm_grad <- (eval(parse(text=paste0("ltm_","",y)))[,scn_grad])-(ltm_init[,scn_grad[1]]) #no dispersal
#   ltm_rand <- (eval(parse(text=paste0("ltm_","",y)))[,scn_rand])-(ltm_init[,scn_rand[1]]) #no dispersal
#   ltm_scenarios <- list(ltm_nodiv, ltm_grad, ltm_rand)
#   
#   plot(NULL, xlim=c(0.5,4.5), ylim=c(-0.25,0.05), xlab="Dispersal rate",ylab="Change of local trait mismatch", xaxt='n', main=paste0(y,"yrs"), cex.axis=1.2, cex.lab=1.2)
#   mtext(c("0%","10%","20%","30%"), side = 1, line = 1, outer = FALSE, at = 1:4, cex=1.2)
#   abline(h=0,lty=3)
#   for (i in 1:length(ltm_scenarios)){
#     points(rep(c(1:4+gap[i]),each=15),ltm_scenarios[[i]],pch=4, cex=0.5,col=color_scn_basic[i])
#     points(c(1:4+gap[i]),apply(ltm_scenarios[[i]],2,median),pch=19, cex=1.5,col=color_scn_basic[i])
#     segments(c(1:4+gap[i]),apply(ltm_scenarios[[i]],2,quantile,probs=.25),
#              c(1:4+gap[i]),apply(ltm_scenarios[[i]],2,quantile,probs=.75),lwd=3,col=color_scn_basic[i])
#   }
...
# gap<-c(-.3,-.1,.1,.3)
# for(y in c(30,40,50)) {
#   ltm_rand <- (eval(parse(text=paste0("ltm_","",y)))[,scn_rand[-1]])-(ltm_init[,scn_rand[-1][1]]) #no dispersal
#   ltm_dist <- (eval(parse(text=paste0("ltm_","",y)))[,scn_dist])-(ltm_init[,scn_dist[1]]) #no dispersal
#   ltm_size <- (eval(parse(text=paste0("ltm_","",y)))[,scn_size])-(ltm_init[,scn_size[1]]) #no dispersal
#   ltm_both <- (eval(parse(text=paste0("ltm_","",y)))[,scn_both])-(ltm_init[,scn_both[1]]) #no dispersal
#   ltm_scenarios <- list(ltm_rand, ltm_dist, ltm_size, ltm_both)
#   
#   plot(NULL, xlim=c(0.5,3.5), ylim=c(-0.25,0.05), xlab="Dispersal rate",ylab="Change of local trait mismatch", xaxt='n', main=paste0(y,"yrs"), cex.axis=1.2, cex.lab=1.2)
#   mtext(c("10%","20%","30%"), side = 1, line = 1, outer = FALSE, at = 1:3, cex=1.2)
#   abline(h=0,lty=3)
#   for (i in 1:length(ltm_scenarios)){
#     points(rep(c(1:3+gap[i]),each=15),ltm_scenarios[[i]],pch=4, cex=0.5,col=color_scn_config[i])
#     points(c(1:3+gap[i]),apply(ltm_scenarios[[i]],2,median),pch=19, cex=1.5,col=color_scn_config[i])
#     segments(c(1:3+gap[i]),apply(ltm_scenarios[[i]],2,quantile,probs=.25),
#              c(1:3+gap[i]),apply(ltm_scenarios[[i]],2,quantile,probs=.75),lwd=3,col=color_scn_config[i])
#   }
#   #legend("topright", cex=.8, legend=c("No diversity","Gradual diversity","Random diversity"), col=2:3, border=NA,fill=c("black","forestgreen","orange"), bty="n", ncol=1)
# }
...
gG_init<-round(seq(from=0.3, to=0, length.out=15),2)
  ...
  for(j in 1:length(scenarios)) {
    if(j==1 && i!=8) {
      ...
      for (scn in scenarios[[j]]) { 
        ...
        lines((1:50), lw$fitted/lw$fitted[1], pch=19,cex=1.5, col=color_scn[scn], lwd=1.5)
      }
    }
  }
}  
...
for (pop in 1:15){
  ...
  #lw_init<-NULL
  for (scn in scn_spat_10) { #choice of scenarios to plot #scn_spat_10
    median_npops<- apply(Npops[,,scn],1,median)
    lw<-loess(median_npops~c(1:(nYears+nInit)),span=0.2)
    #lw_init<-c(lw_init,lw$fitted[1])
    #lines((1:(nYears+nInit)), lw$fitted/lw_init[1], pch=19,cex=1.5, col=color_scn[scn], lwd=2)
...
#################################################################
######### TEMPORAL AVERAGE CV GROWTH POTENTIAL (Fig. 6) #########
#################################################################
...
metapop_CVPlot <- function(metric, ylim, ylab, scenarios, immi) {
 if(scenarios == "diversity") {
   scenarios <- list(scn_nodiv, scn_grad, scn_rand)
   if(immi == TRUE) {
     plot(NULL, xlim=c(0,(nInit+nYears)), ylim=ylim, xlab="Years",ylab=ylab, main="IMMIGRANTS", cex.lab=1.2,cex.axis=1.2)
   ...
   if(immi == TRUE) {
     plot(NULL, xlim=c(0,(nInit+nYears)), ylim=ylim, xlab="Years",ylab=ylab, main="20%disp-IMMIGRANTS", cex.lab=1.2,cex.axis=1.2)
   }else{
...
metapop_CVPlot(cv_years_immi, c(0.06,.1),"Averaged local CV growth potential", "diversity",TRUE)
metapop_CVPlot(cv_years_immi, c(0.06,.1),"Averaged local CV growth potential", "config",TRUE)
...
plot(NULL, xlim=c(0,16), ylim=c(-1,2), xlab="Population",ylab="Ratio Immigrants/Emigrants (log)", xaxt='n')
...
visNetworkPlot <- function(scenario){
  ...
  if(scenario %in% scn_nodiv) {
     ...
     gradient_color <- rep(colors[8],15) #rep(gradient_color_gradual[1],15)
  }
  if(scenario %in% scn_grad) {
    #for scn gradual diversity
    gradient_color <- colors #c("#009B9F", "#00A6AA", "#1EB2B5", "#62BDC0",
...
####################################
### Fig.S16 REPRODUCTIVE SUCCESS ###
####################################

##focus on scn dispersal 10% random diversity simple spatial configuration

load(paste0("results/FITNESS/FITNESS",30341211,"_50.RData"))
